    ...
    <main className="w-full flex min-h-screen flex-col font-sans bg-[#1a1a1a]">
      ...
      <section className="max-w-7xl mx-auto w-full px-6 md:px-12 pb-32 relative z-20">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-16">
          {siteConfig.films.map((film, index) => (
             <motion.div
               key={film.id}
             ...
             >
                {film.ready ? (
                  <a
                     ...
                     className={`block w-full mb-8 overflow-hidden shadow-2xl cursor-pointer bg-black ${imageCardClass}`}
                  >
                     <IkImage
                       src={film.thumbnail}
                       alt={film.title}
                  ...
                  <div className={`block w-full mb-8 overflow-hidden shadow-2xl bg-black relative ${imageCardClass}`}>
                     <IkImage
                       src={film.thumbnail}
                       alt={film.title}
                       fill
                     ...
                     <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/40">
                        <span className="text-white/90 uppercase tracking-[0.3em] text-xs font-bold border border-white/40 px-4 py-2">
                          Coming Soon
                        </span>
                     </div>
                  </div>
