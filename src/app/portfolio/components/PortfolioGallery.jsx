    ...
    <section className="w-full flex-grow bg-[#faf9f6] pb-24 relative overflow-hidden">
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 md:px-12 min-h-[600px] relative z-10">
            ...
            {filteredItems.map((item) => (
              ...
              >
                <Link href={`/portfolio/${item.slug}`} className="absolute inset-0 block">
                  <IkImage
                    src={item.thumbnail}
                    alt={`${item.coupleNames} — ${item.category}`}
                    fill
                    preset="thumbnail"
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col items-center justify-center text-center px-4">
                    <p className="text-white font-script text-2xl md:text-3xl mb-2 translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                      {item.coupleNames}
                    </p>
                    <div className="w-12 h-[1px] bg-white mx-auto scale-0 group-hover:scale-100 transition-transform duration-500 delay-100"></div>
                    <p className="text-white/80 uppercase tracking-[0.2em] text-xs mt-4 translate-y-4 group-hover:translate-y-0 transition-transform duration-500 delay-75">
                      {item.category}
                    </p>
                  </div>
                </Link>
        ...
        {filteredItems.length === 0 && (
          <div className="w-full text-center py-24 text-gray-500 italic font-serif text-xl border-dashed border border-[#e2d5c5]/50 flex items-center justify-center h-48">
